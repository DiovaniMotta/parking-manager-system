...
public class PersistenciaClienteFactory extends PersistenceFactory {
	...
	public <T extends Entidade> Persistence<T> open(Persistencia persistencia) throws IOException, ValidatePropertyException {
		if(persistencia == Persistencia.BINARIO)
			return (Persistence<T>) new PersistenceSessionBinaryCliente();
		return (Persistence<T>) new PersistenceSessionSequenceCliente();
	}
